...
export function verifyIsolation({ context, namespace, release, k, deployment, pod, values }) {
  ...
  const waitForJob = (name, timeout) => {
    try {
      command(["wait", "--for=condition=complete", "job/" + name, "--timeout=" + timeout]);
    } catch (error) {
  ...
  const labels = {
    "app.kubernetes.io/name": deployment.spec.template.metadata.labels["app.kubernetes.io/name"],
    "app.kubernetes.io/instance": release,
    "helmforge.dev/isolation-fixture": "negative",
  };
  const admissionKey = deployment.spec.template.spec.volumes.find((item) => item.name === "admission-key").secret
    .secretName;
  const runtime = deployment.spec.template.spec.volumes.find((item) => item.name === "runtime").configMap.name;
  const image = values.admission.image.repository + ":" + values.admission.image.tag;
  const jobName = "twenty-negative-admission",
    policyName = "twenty-negative-admission";
  const code =
          ...
          from: [
            {
              podSelector: {
                matchLabels: {
                  "app.kubernetes.io/name": "twenty-admission",
                  "app.kubernetes.io/instance": release,
                },
              },
            },
  ...
  try {
    ...
    assert.match(command(["logs", "job/" + jobName]), /PASS unenforced native-port isolation blocks enrollment/);
  } finally {
    command(["delete", "job", jobName, "--ignore-not-found", "--wait=true", "--timeout=30s"]);
  ...
  try {
    ...
    assert.match(command(["logs", "job/" + peerName]), /PASS public health reachable and native listener denied/);
  } finally {
    command(["delete", "job", peerName, "--ignore-not-found", "--wait=true", "--timeout=30s"]);
  }
  console.log(
