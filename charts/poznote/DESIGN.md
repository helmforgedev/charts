...
```text
ghcr.io/timothepoznanski/poznote:6.92.0
```

The tag maps to the upstream `6.92.0` release and publishes Linux `amd64` and `arm64` manifests.

Upstream also publishes a `6.92.0-rootless` variant. It is not the chart
