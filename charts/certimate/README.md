...
This chart packages the official `certimate/certimate:v0.4.33` image and follows the upstream container contract: HTTP on port `8090` and durable PocketBase data under `/app/pb_data`.
...
Certimate 0.4.33 adds deployment providers and SSH REPL hooks, and fixes DNS,
Kubernetes Secret creation, and uniCloud deployment integrations. It does not
change the container port, storage path, or chart configuration contract.
...
```text
Image: certimate/certimate:v0.4.33
