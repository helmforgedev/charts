...
{{- define "immich.valkeyHost" -}}{{- if include "immich.valkeyInternalEnabled" . -}}{{- if eq .Values.valkey.architecture "replication" -}}{{ .Release.Name }}-valkey-primary{{- else if or (eq .Values.valkey.architecture "standalone") (eq .Values.valkey.architecture "cluster") -}}{{ .Release.Name }}-valkey-client{{- else -}}{{- fail "Immich internal cache supports valkey.architecture=standalone, replication, or cluster" -}}{{- end -}}{{- else -}}{{ .Values.valkey.external.host }}{{- end -}}{{- end -}}
