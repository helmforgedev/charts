...
- HelmForge pins the Docker Official image explicitly to `11.3.10-php8.5-apache-bookworm`, which includes PHP 8.5 and the Drupal-required database extensions.
...
- **Pinned production image** — uses `drupal:11.3.10-php8.5-apache-bookworm`, which matches current Drupal 11.3 support for PHP 8.5 and keeps the Debian base explicit
